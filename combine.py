...
# Modified get_data() to use an end date of tomorrow so that today's data is attempted
def get_data(ticker):
    # Set the end date as tomorrow's date (yfinance's end is exclusive)
...
# Visualization simply plots the fetched data
...
def linear_regression_prediction():
    ...
    if ticker:
        ...
        if df.empty:
            ...
            return
        # Use the stock's actual first trading date as the start date
        stock_first_date = df['Date'].min().date()
        today_date = pd.Timestamp.today().date()
        # Set date inputs with min_value as the stock's first trading date
        start_date = st.date_input("Select Start Date", value=stock_first_date, min_value=stock_first_date)
        end_date = st.date_input("Select End Date", value=today_date, max_value=today_date)
...
# (Long-Term Prediction) – Unchanged
# =========================
def lstm_prediction():
